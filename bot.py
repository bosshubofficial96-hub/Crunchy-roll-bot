...
class PremiumEmoji:
    ...
    # ── Map: name → (telegram_custom_emoji_id, unicode_fallback) ─────────────
...
class Database:
    """Full SQLite database manager with all tables"""
    ...
    def _init_tables(self):
        tables = [
            ...
            # Custom commands (text + python code)
...
class VideoTools:
    ...
    # ── Thumbnail Embed (Permanent) ───────────────────────────────────────────

    @staticmethod
    async def embed_thumbnail(video_path: str, thumb_path: str) -> Optional[str]:
        """
        Permanently embed a thumbnail into an MP4 file using FFmpeg.
    ...
    async def trim_video(input_path: str, start: str, end: str) -> Optional[str]:
        """Trim video between start and end timestamps (HH:MM:SS format)."""
...
# ══════════════════════════════════════════════════════════════════════════════
#  SECTION 6: CRUNCHYROLL API (Parse + Mock Download)
# ══════════════════════════════════════════════════════════════════════════════
...
class DownloadManager:
    ...
    async def process(self, queue_id: int, uid: int, url: str,
        ...
        try:
            ...
            # ── Simulated download (replace with real HLS/HTTP download) ──────
            # In production: use yt-dlp or ffmpeg to download HLS stream
            ...
            # For demo: skip actual encode if placeholder
            encoded_path = raw_path
            # Uncomment when real download works:
            # encoded_path = await VideoTools.encode_video(raw_path, quality, preset, enc_cb)
            # if not encoded_path:
...
# ══════════════════════════════════════════════════════════════════════════════
#  SECTION 8: SANDBOX CODE EXECUTOR (for custom Python commands)
# ══════════════════════════════════════════════════════════════════════════════
...
class CodeSandbox:
    ...
    BLOCKED_PATTERNS = [
        ...
        r"exec\s*\(", r"eval\s*\(",
        r"__import__", r"__builtins__", r"globals\s*\(",
...
class AuthChecker:
    ...
    async def check_user(bot, uid: int, db: Database) -> Tuple[bool, List[str]]:
        ...
        if not db.get_setting("force_sub_enabled", "False") == "True":
            return True, []
        ...
        # Check auth channel from settings
...
class CrunchyrollBot:
    ...
    # Format: /cr <url> [quality] [encode_preset]
    ...
    async def cmd_cr(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        # Parse optional quality and preset from args
        quality = None
        preset  = None
        ...
        # If quality specified, validate premium requirement
        ...
        # If no quality selected, show keyboard
        if not quality:
    ...
    async def cmd_thumb(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        """Set custom thumbnail (reply with image)."""
        msg = update.message
    ...
    async def cmd_addcmd(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        """
        /addcmd <name> <type> <content>
        Types:
          text     — plain text response
          markdown — markdown response
          python   — run Python code (sandboxed, no imports)
        """
        if len(ctx.args) < 3:
            await update.message.reply_text(
                f"{Em.CODE} Usage: `/addcmd <name> <text|markdown|python> <content>`\n\n"
                f"**Python example:**\n`/addcmd roll python print(f'You rolled: {{__import__(\"random\").randint(1,6)}}')`\n"
    ...
    async def handle_text(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        """Handles custom /<command> calls from database."""
        msg = update.message
                    ...
                    try:
                        ...
                        with open(result, "rb") as fh:
                            ...
                            await self.application.bot.send_video(
                                chat_id=uid,
                                video=InputFile(fh, filename=fname),
                                caption=(
...
╚═══════════════════════════════════════════════════════════════════════════════╝
    ...
    if not Config.validate():
        print("❌ Please configure your BOT_TOKEN environment variable!")
