...
class PremiumEmoji:
    ...
    def get(cls, name: str) -> str:
        """Return custom emoji tag or Unicode fallback."""
...
E = PremiumEmoji  # short alias

# ── Simpler fallback-only emoji constants ────────────────────────────────────
class Em:
...
class CrunchyrollBot:
    ...
    # ══ Helper to get message from update (FIXED for callbacks) ═══════════════
    def _get_message(self, update: Update):
        """Safely get message from update, whether it's a message or callback query"""
        if update.callback_query:
            return update.callback_query.message
        return update.message

    def _get_user_id(self, update: Update):
    ...
    async def cmd_start(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        kb = [
            ...
            [InlineKeyboardButton(f"{Em.ADMIN} Admin Panel", callback_data="show_admin")],
        ]
    ...
    async def cmd_cr(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.downloader.cr.is_valid_url(url):
            await message.reply_text(f"{Em.ERROR} Invalid Crunchyroll URL.")
            return
    ...
    async def cmd_subscribe(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await message.reply_invoice(
            ...
            payload=f"sub_{plan}_{self._get_user_id(update)}_{uuid.uuid4().hex[:8]}",
            provider_token="",
            currency="XTR",
    ...
    async def cmd_redeem(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        message = self._get_message(update)
        if not ctx.args:
            await message.reply_text(f"{Em.REDEEM} Usage: `/redeem <CODE>`", parse_mode=ParseMode.MARKDOWN)
            return
        code = ctx.args[0].strip()
        ok, msg = self.db.redeem_code(code, self._get_user_id(update))
        if ok:
            await message.reply_text(
                f"{Em.SUCCESS} **Code redeemed!** Activated: {msg}", parse_mode=ParseMode.MARKDOWN)
        else:
            await message.reply_text(f"{Em.ERROR} {msg}", parse_mode=ParseMode.MARKDOWN)
    ...
    async def cmd_gift(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if not is_pm and not self.db.is_admin(uid):
            await message.reply_text(f"{Em.PREMIUM} Gifting requires premium.", parse_mode=ParseMode.MARKDOWN)
            return
        if len(ctx.args) < 2:
            await message.reply_text(
        ...
        try:
            ...
            await message.reply_text(f"{Em.ERROR} Invalid user ID.", parse_mode=ParseMode.MARKDOWN)
            return
        if plan not in Config.SUBSCRIPTION_PRICES:
            await message.reply_text(f"{Em.ERROR} Invalid plan.", parse_mode=ParseMode.MARKDOWN)
            return
        days = Config.SUBSCRIPTION_DAYS[plan]
    ...
    async def _claim_gift(self, update: Update, ctx, gift_code: str):
        ...
        r   = c.fetchone()
        if not r:
            await message.reply_text(f"{Em.ERROR} Invalid gift code.")
            return
        r = dict(r)
        if r["claimed"]:
            await message.reply_text(f"{Em.ERROR} This gift was already claimed.")
            return
        if r["to_user"] != uid and r["to_user"] != 0:
            await message.reply_text(f"{Em.ERROR} This gift is not for you.")
            return
    ...
    async def cmd_queue(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        for it in items:
            ...
            short_url = it["url"][-30:] if len(it["url"]) > 30 else it["url"]
            lines.append(
    ...
    async def cmd_cancel(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        message = self._get_message(update)
        if not ctx.args:
            await message.reply_text(f"{Em.WARNING} Usage: `/cancel <id>`", parse_mode=ParseMode.MARKDOWN)
            return
        try:
            qid = int(ctx.args[0])
            if self.db.cancel_queue_item(qid, self._get_user_id(update)):
                await message.reply_text(f"{Em.SUCCESS} Download #{qid} cancelled.")
            else:
                await message.reply_text(f"{Em.ERROR} Cannot cancel (already processing or not yours).")
        except ValueError:
            await message.reply_text(f"{Em.ERROR} Invalid ID.")
    ...
    async def cmd_history(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if not hist:
            await message.reply_text(f"{Em.INFO} No history yet. Use /cr to start!")
            return
    ...
    async def cmd_stats(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await message.reply_text(
            ...
            f"**Profile:**\n"
            f"├ Name: {user.get('first_name','?') if user else '?'}\n"
            f"├ Premium: {'✅ ' + plan.upper() if is_pm else '❌ Free'}\n"
            f"├ Expires: {expiry.strftime('%Y-%m-%d') if expiry and is_pm else 'N/A'}\n"
            f"└ Stars: {user.get('stars_balance',0) if user else 0:,}\n\n"
            f"**Usage:**\n"
            f"├ Today: {daily}/{limit}\n"
            f"├ All-time: {user.get('total_downloads',0) if user else 0:,}\n"
            f"├ Data: {round((user.get('total_size',0) if user else 0)/(1024**3),3)} GB\n"
    ...
    async def cmd_settings(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        user = self.db.get_user(uid)
        kb   = [
            [InlineKeyboardButton(f"{Em.QUALITY} Quality: {user.get('default_quality','720p') if user else '720p'}",
                                   callback_data="cfg_quality")],
            [InlineKeyboardButton(f"{Em.ENCODE} Preset: {user.get('encode_preset','balanced') if user else 'balanced'}",
                                   callback_data="cfg_preset")],
            [InlineKeyboardButton("🔔 Notifications: " + ("ON" if user and user.get("notify_complete") else "OFF"),
    ...
    async def cmd_favorites(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if not favs:
            await message.reply_text(f"{Em.HEART} No favorites yet.")
            return
    ...
    async def cmd_watchlist(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if not wl:
            await message.reply_text(f"{Em.WATCH} Watchlist empty. Add anime with /addwatch.")
            return
    ...
    async def cmd_leaderboard(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        for i, u in enumerate(lb):
            lines.append(
                ...
                f"{round((u.get('total_size',0) if u else 0)/(1024**3),2)} GB")
        await message.reply_text("\n".join(lines), parse_mode=ParseMode.MARKDOWN)
    ...
    async def cmd_feedback(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await message.reply_text(f"{Em.ERROR} Rating must be 1-5.")
            return
        msg   = " ".join(ctx.args[1:])
        stars = "⭐" * rating + "☆" * (5 - rating)
        self.db.add_feedback(self._get_user_id(update), rating, msg)
        await message.reply_text(
    ...
    async def cmd_schedule(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await message.reply_text(f"{Em.ERROR} Invalid time format. Use HH:MM")
            return

        sid = self.db.schedule_download(self._get_user_id(update), url, quality, preset, run_at.isoformat())
        await message.reply_text(
    ...
    async def cmd_admin(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        kb = [
            ...
            [InlineKeyboardButton("🔙 Back", callback_data="show_home")],
        ]
        ...
        await message.reply_text(
            ...
            f"Use the buttons below for detailed controls:",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup(kb))
    ...
    async def cmd_ban(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        message = self._get_message(update)
        if len(ctx.args) < 2:
            await message.reply_text(f"{Em.WARNING} Usage: `/ban <uid> <reason> [days]`", parse_mode=ParseMode.MARKDOWN)
            return
        try:
            uid    = int(ctx.args[0])
            days   = int(ctx.args[-1]) if ctx.args[-1].isdigit() else 0
            reason = " ".join(ctx.args[1:-1]) if days else " ".join(ctx.args[1:])
            self.db.ban_user(uid, self._get_user_id(update), reason, days)
            dur = f" for {days} days" if days else " permanently"
            await message.reply_text(f"{Em.BAN} User `{uid}` banned{dur}.\nReason: {reason}", parse_mode=ParseMode.MARKDOWN)
            try:
                await ctx.bot.send_message(uid, f"{Em.WARNING} You have been banned{dur}.\nReason: {reason}")
            except:
                pass
        except ValueError:
            await message.reply_text(f"{Em.ERROR} Invalid user ID.")

    @mod_only
    async def cmd_unban(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        message = self._get_message(update)
        if not ctx.args:
            await message.reply_text(f"{Em.WARNING} Usage: `/unban <uid>`", parse_mode=ParseMode.MARKDOWN)
            return
        try:
            uid = int(ctx.args[0])
            self.db.unban_user(uid)
            await message.reply_text(f"{Em.UNBAN} User `{uid}` unbanned.", parse_mode=ParseMode.MARKDOWN)
            try:
    ...
    async def cmd_warn(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        message = self._get_message(update)
        if len(ctx.args) < 2:
            await message.reply_text(f"{Em.WARNING} Usage: `/warn <uid> <reason>`", parse_mode=ParseMode.MARKDOWN)
            return
        try:
            uid    = int(ctx.args[0])
            ...
            try:
                ...
                pass
            if warns >= 5:
                self.db.ban_user(uid, self._get_user_id(update), "Auto-ban: 5 warnings", 30)
                await message.reply_text(f"{Em.BAN} User auto-banned (5 warnings).")
        except ValueError:
            await message.reply_text(f"{Em.ERROR} Invalid user ID.")
    ...
    async def cmd_broadcast(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        message = self._get_message(update)
        if not ctx.args:
            await message.reply_text(f"{Em.WARNING} Usage: `/broadcast <message>`", parse_mode=ParseMode.MARKDOWN)
            return
        text   = " ".join(ctx.args)
        users  = self.db.get_all_users()
        status = await message.reply_text(f"{Em.LOADING} Broadcasting to {len(users):,} users…")
        sent, fail = 0, 0
        for uid in users:
            try:
    ...
    async def cmd_addpremium(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            await message.reply_text(f"{Em.ERROR} Invalid arguments.")
            return
        if self.db.add_premium(uid, plan, days):
            ...
            await message.reply_text(f"{Em.ERROR} Failed.")
    ...
    async def cmd_gencode(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            return
        code = secrets.token_hex(4).upper()
        if self.db.create_redeem_code(code, plan, days, max_uses, self._get_user_id(update), exp_days):
            await message.reply_text(
            ...
            await message.reply_text(f"{Em.ERROR} Failed to create code.")
    ...
    async def cmd_queueall(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if not items:
            await message.reply_text(f"{Em.QUEUE} Queue is empty.")
            return
    ...
    async def cmd_clearqueue(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        await message.reply_text(f"{Em.SUCCESS} Cleared {count} items from queue.")

    @admin_only
    async def cmd_maintenance(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        message = self._get_message(update)
        if not ctx.args or ctx.args[0] not in ("on", "off"):
            await message.reply_text(f"{Em.WARNING} Usage: `/maintenance <on|off>`", parse_mode=ParseMode.MARKDOWN)
            return
        val = "True" if ctx.args[0] == "on" else "False"
        self.db.set_setting("maintenance_mode", val)
        await message.reply_text(f"{Em.SUCCESS} Maintenance mode: **{ctx.args[0].upper()}**", parse_mode=ParseMode.MARKDOWN)
    ...
    async def cmd_authgroup(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "list":
            ...
            if not groups:
                await message.reply_text(f"{Em.INFO} No auth groups configured.")
                return
            ...
            try:
                ...
                self.db.add_auth_group(gid, name, link, self._get_user_id(update))
                await message.reply_text(f"{Em.SUCCESS} Auth group `{gid}` added.", parse_mode=ParseMode.MARKDOWN)
            except ValueError:
                await message.reply_text(f"{Em.ERROR} Invalid group ID.")
        elif action == "remove" and len(ctx.args) >= 2:
            try:
                gid = int(ctx.args[1])
                self.db.remove_auth_group(gid)
                await message.reply_text(f"{Em.SUCCESS} Auth group `{gid}` removed.", parse_mode=ParseMode.MARKDOWN)
            except ValueError:
                await message.reply_text(f"{Em.ERROR} Invalid group ID.")
        elif action in ("on", "off"):
            val = "True" if action == "on" else "False"
            self.db.set_setting("force_sub_enabled", val)
            await message.reply_text(f"{Em.SUCCESS} Force-subscribe: **{action.upper()}**", parse_mode=ParseMode.MARKDOWN)
        else:
            await message.reply_text(f"{Em.ERROR} Invalid arguments.")
    ...
    async def cmd_logs(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        rows = c.fetchall()
        if not rows:
            await message.reply_text(f"{Em.LOG} No download logs yet.")
            return
    ...
    async def cmd_restart(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        message = self._get_message(update)
        await message.reply_text(f"{Em.LOADING} Restarting…")
        os._exit(0)
    ...
    async def cmd_addcmd(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if cmd_type not in ("text", "markdown", "python"):
            await message.reply_text(f"{Em.ERROR} Type must be: text, markdown, or python")
            return

        if cmd_type == "python":
            ok, out = CodeSandbox.run(content, {"user_id": self._get_user_id(update)})
            if not ok:
                await message.reply_text(
        ...
        if self.db.add_custom_cmd(cmd_name, response, code, cmd_type, self._get_user_id(update)):
            await message.reply_text(
    ...
    async def cmd_editcmd(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if self.db.update_custom_cmd(cmd_name, response, code, cmd_type):
            await message.reply_text(f"{Em.SUCCESS} `/{cmd_name}` updated!", parse_mode=ParseMode.MARKDOWN)
        else:
            await message.reply_text(f"{Em.ERROR} Command not found.", parse_mode=ParseMode.MARKDOWN)

    @admin_only
    async def cmd_delcmd(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        message = self._get_message(update)
        if not ctx.args:
            await message.reply_text(f"{Em.WARNING} Usage: `/delcmd <name>`", parse_mode=ParseMode.MARKDOWN)
            return
        if self.db.remove_custom_cmd(ctx.args[0].lower()):
            await message.reply_text(f"{Em.SUCCESS} Command `/{ctx.args[0]}` removed.", parse_mode=ParseMode.MARKDOWN)
        else:
            await message.reply_text(f"{Em.ERROR} Command not found.", parse_mode=ParseMode.MARKDOWN)
    ...
    async def cmd_listcmds(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if not cmds:
            await message.reply_text(f"{Em.INFO} No custom commands.")
            return
    ...
    async def callback_handler(self, update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        # Store the callback query in update for helper methods
        update.callback_query = q

        # Quality selection from /cr
        ...
        # Navigation - Call the actual command methods (they now work with callbacks)
            ...
            await self.cmd_admin(update, ctx)
        elif data == "cancel":
        ...
        # Admin panel callbacks
            ...
            await self.cmd_queueall(update, ctx)
        elif data == "admin_codes":
            await q.edit_message_text(
                f"{Em.REDEEM} **Redeem Code Generator**\n\n"
                f"Use: `/gencode <plan> <days> [max_uses] [expires_days]`\n\n"
                f"Example: `/gencode monthly 30 5 7`",
                parse_mode=ParseMode.MARKDOWN)
        elif data == "admin_broadcast":
            await q.edit_message_text(
                f"{Em.BROAD} **Broadcast Message**\n\n"
                f"Send: `/broadcast <your message>`\n\n"
                f"⚠️ This will send to ALL users!",
                parse_mode=ParseMode.MARKDOWN)
        elif data == "admin_restart":
            await q.edit_message_text(f"{Em.LOADING} Restarting bot...")
            os._exit(0)
...
║          ✅ ALL ERRORS FIXED | CALLBACKS WORKING | ADMIN PANEL READY        ║
║          📦 2GB FILE SUPPORT: {}                         ║
╚═══════════════════════════════════════════════════════════════════════════════╝""".format("ENABLED" if Config.USE_MT_PROTO else "DISABLED"))
